...
# Não existe diretamente a função `prompt` em Crystal, mas usamos `gets` para capturar entradas.
def entrada_saida
  print "Digite seu nome: " # Captura o nome do usuário
  nome = gets.chomp
  puts "Olá, #{nome}"
end
...
def manipulacao_strings
  ...
  contem = palavra.includes?("grama") # Verifica se a palavra contém "grama"
...
def exemplo_this
  class Carro
    def initialize(modelo)
      @modelo = modelo # 'this' em Crystal é representado por '@'
    end
...
def exemplo_encapsulamento
  ...
  conta.depositar(500)
  puts "Saldo da conta: #{conta.get_saldo}"
end
...
def exemplo_arraylist
  lista = ["Java", "Python", "C++"]

  puts "Tamanho da lista: #{lista.size}" # Exibe o tamanho da lista
